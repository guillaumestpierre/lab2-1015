...
void ajouterFilm(Film& film, ListeFilms& listeFilms)
{
	if (listeFilms.nElements == listeFilms.capacite)
	{
		...
		for (int i = 0; i < listeFilms.nElements; i++)
		{
			nouveauPointeurTableau[i] = new Film(*listeFilms.elements[i]);
		}
...
void ajouterActeur(Acteur film, ListeActeurs& listeFilms)
{
	if (listeFilms.nElements == listeFilms.capacite)
	{
		int nouvelleCapacite;
		if (listeFilms.capacite != 0)
			nouvelleCapacite = listeFilms.capacite * 2;
		else
			nouvelleCapacite = 2;

		Acteur** nouveauPointeurTableau = new Acteur * [nouvelleCapacite];
...
Acteur* lireActeur(istream& fichier, ListeFilms& listeFilms)
{
	...
	{
		retour = pointeurActeur;
		cout << "Acteur existant : " << pointeurActeur->nom << endl;
	}
	else
	{
		Acteur* pointeurNouvelActeur = new Acteur(acteur);
		retour = pointeurNouvelActeur;
		//cout << "Creation nouvel acteur : " << pointeurNouvelActeur->nom << endl;
	}

	return retour; //TODO: Retourner un pointeur soit vers un acteur existant ou un nouvel acteur ayant les bonnes informations, selon si l'acteur existait déjà.  Pour fins de débogage, affichez les noms des acteurs crées; vous ne devriez pas voir le même nom d'acteur affiché deux fois pour la création.
}
...
Film* lireFilm(istream& fichier, ListeFilms& listeFilms)
{
	...
	ajouterFilm(film, listeFilms);

	for (int i = 0; i < film.acteurs.nElements; i++) {
		pointeurListeActeurs[i] = new Acteur(*lireActeur(fichier, listeFilms)); //TODO: Placer l'acteur au bon endroit dans les acteurs du film.
		//TODO: Ajouter le film à la liste des films dans lesquels l'acteur joue.
		ajouterFilm(film, pointeurListeActeurs[i]->joueDans);
	}
	for (int i = 0; i < listeFilms.nElements; i++)
	{
		auto test = (*listeFilms.elements[i]).titre == film.titre;
...
ListeFilms creerListe(string nomFichier)
{
	...
	for (int i = 0; i < nElements; i++) {
		pointeurListeFilms[i] = new Film (*lireFilm(fichier, listeFilms)); //TODO: Ajouter le film à la liste.
	}
